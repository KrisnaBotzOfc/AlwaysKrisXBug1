...
const { imageToWebp, videoToWebp, writeExifImg, writeExifVid } = require('./ラ-Library/exif')
const { smsg, isUrl, generateMessageTag, getBuffer, getSizeMedia, fetch, await, sleep, reSize } = require('./ラ-Library/myfunc')
const { default: clientConnect, delay, makeCacheableSignalKeyStore, useMultiFileAuthState, DisconnectReason, fetchLatestBaileysVersion, generateForwardMessageContent, prepareWAMessageMedia, generateWAMessageFromContent, generateMessageID, downloadContentFromMessage, makeInMemoryStore, jidDecode, proto, Browsers} = require("@whiskeysockets/baileys")
...
const {  state, saveCreds } =await useMultiFileAuthState(`./session`)
   ...
   if (pairingCode && !client.authState.creds.registered) {
      ...
      if (!!phoneNumber) {
        ...
        phoneNumber = await question(chalk.bgBlack(chalk.greenBright(`For example: +62xxx : `)));
        phoneNumber = phoneNumber.replace(/[^0-9]/g, '');
    }
...
client.ev.on("connection.update",async  (s) => {
        ...
        if (connection == "open") {
            console.log(chalk.magenta(`< # > Terkoneksi Ke Bot! `))
        }
        if (
            connection === "close" &&
            lastDisconnect &&
